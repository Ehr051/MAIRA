...
function debeIgnorarAtajo(e) {
    // 🎮 NUEVO: Ignorar si está activo el modo 3D (dejar que controles 3D manejen teclas)
    if (window.maira3DActive || document.querySelector('#maira-3d-container.active')) {
        // Solo permitir Esc para salir del 3D
        if (e.key !== 'Escape') {
            return true;
        }
    }
    ...
    // Ignorar si se está editando texto en el map
...
function duplicarElemento() {
    if (window.elementoSeleccionado) {
        ...
        if (elementoOriginal instanceof L.Marker) {
            ...
            // ✅ FIX #8: Copiar EXPLÍCITAMENTE todas las propiedades custom
            const nombreOriginal = elementoOriginal.options.nombre || 'Elemento';
            const nombreCopia = nombreOriginal.includes('(copia)') ? nombreOriginal : nombreOriginal + ' (copia)';
            
            console.log("📋 Copiando elemento con propiedades:", {
                sidc: elementoOriginal.options.sidc,
                tipo: elementoOriginal.options.tipo,
                caracteristica: elementoOriginal.options.caracteristica,
                designacion: elementoOriginal.options.designacion,
                dependencia: elementoOriginal.options.dependencia,
                tipoVehiculo: elementoOriginal.options.tipoVehiculo,
                magnitud: elementoOriginal.options.magnitud,
                equipo: elementoOriginal.options.equipo,
                jugador: elementoOriginal.options.jugador
            });
            
            nuevoElemento = L.marker(newLatLng, {
                icon: elementoOriginal.options.icon, // Copiar icono
                draggable: true,
                // ✅ CRÍTICO: Copiar TODAS las propiedades custom
                sidc: elementoOriginal.options.sidc,
                tipo: elementoOriginal.options.tipo,
                caracteristica: elementoOriginal.options.caracteristica,
                designacion: elementoOriginal.options.designacion,
            ...
            console.log("✅ Elemento copiado con todas las propiedades");
        }
        ...
        if (nuevoElemento && window.calcoActivo) {
            ...
            if (window.mostrarNotificacion) {
                mostrarNotificacion('📋 Elemento duplicado con todas sus propiedades', 'success');
            }
        }
    } else {
        if (window.mostrarNotificacion) {
