...
class TerrainGenerator3D {
    ...
    async addVegetationByFeatures(enrichedPoints) {
        ...
        try {
            ...
            console.log(`🎯 Tipos de objetos creados:`, vegetationObjects.filter(o => o).map(o => o.userData?.vegetationType || 'unknown').slice(0, 10));
            
            // ✅ CRÍTICO: Los objetos ya son THREE.Object3D directos, NO tienen propiedad .mesh
            if (this.maira3DSystem && this.maira3DSystem.scene) {
                vegetationObjects.forEach(obj => {
                    if (obj && obj.isObject3D) {
                        console.log(`🌳 Añadiendo ${obj.userData?.vegetationType} a escena en posición:`, obj.position);
                        this.maira3DSystem.scene.add(obj);
                    }
                });
        ...
        try {
            ...
            if (this.vegetationInstancer && this.useInstancing && !forceIndividualMeshes) {
                ...
                if (instances && instances.length > 0) {
                    ...
                    // 🔥 FIX: Retornar el mesh directamente, no un wrapper
                    // Agregar metadata al userData del mesh
                    instance.userData = {
                        id,
                        type: 'vegetation',
                        vegetationType: type,
                    ...
                    return instance; // ✅ Retornar THREE.Object3D directamente
                }
            }
            ...
            if (this.modelLoader) {
                ...
                if (mesh) {
                    ...
                    // 🔥 FIX: Agregar metadata al userData
                    mesh.userData = {
                        id,
                        type: 'vegetation',
                        vegetationType: type,
                    ...
                    // ✅ Retornar mesh directamente, no wrapper
                    return mesh;
                }
            }
    ...
    clearVegetation() {
        ...
        this.vegetationObjects.forEach(veg => {
            // ✅ FIX: Ahora veg ES el mesh directamente, no tiene propiedad .mesh
            if (veg && veg.isObject3D) {
                if (this.maira3DSystem && this.maira3DSystem.scene) {
                    this.maira3DSystem.scene.remove(veg);
                }
                // Liberar geometría/material si es necesario
                if (veg.geometry) veg.geometry.dispose();
                if (veg.material) {
                    if (Array.isArray(veg.material)) {
                        veg.material.forEach(mat => mat.dispose());
                    } else {
                        veg.material.dispose();
                    }
                }
            }
        });
