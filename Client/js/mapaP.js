...
function inicializarEventosMapa() {
    mapa.on('click', function(e) {
        ...
        if (elementosEnPunto.length > 0) {
            seleccionarElemento(elementosEnPunto[0]);
        } else {
            deseleccionarElemento();
        }
    });
...
function inicializarControlesCuadricula() {
    ...
    var gridWidthSlider = document.getElementById('gridWidthSlider');

    if (opacitySlider) {
        opacitySlider.addEventListener('input', actualizarEstiloCuadricula);
    } else {
        console.warn("Elemento 'opacitySlider' no encontrado");
    }

    if (colorSelector) {
        colorSelector.addEventListener('input', actualizarEstiloCuadricula);
    } else {
        console.warn("Elemento 'colorSelector' no encontrado");
    }

    if (gridWidthSlider) {
        gridWidthSlider.addEventListener('input', actualizarEstiloCuadricula);
    } else {
        console.warn("Elemento 'gridWidthSlider' no encontrado");
    }
}

// Llama a esta función después de inicializar el mapa
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('opacitySlider').addEventListener('input', actualizarEstiloCuadricula);
    document.getElementById('colorSelector').addEventListener('input', actualizarEstiloCuadricula);
    document.getElementById('gridWidthSlider').addEventListener('input', actualizarEstiloCuadricula);
    ...
    if (coordenadasCheckbox) {
        ...
        console.warn("Elemento 'coordenadasCheckbox' no encontrado");
    }

    // Event listeners para controles de cuadrícula
    var opacitySlider = document.getElementById('opacitySlider');
    var colorSelector = document.getElementById('colorSelector');
    var gridWidthSlider = document.getElementById('gridWidthSlider');
    ...
    var tiposCuadricula = document.querySelectorAll('#tipoCuadriculaBtn > div');
...
// Exportación de funciones y variables para uso en otros archivos
window.mapa = mapa;
window.calcoActivo = calcoActivo;
window.elementoSeleccionado = elementoSeleccionado;
