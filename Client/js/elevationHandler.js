// elevationHandler.js - Sistema corregido para mini-tiles v3.0

console.log('🏔️ Inicializando elevation handler para mini-tiles v3.0...');

    // Configuración de URLs para mini-tiles
...
window.obtenerElevacion = async function(lat, lng) {
    try {
        ...
        return elevation || calculateArgentineElevation(lat, lng);
...
// Inicializar automáticamente el sistema
initializeElevationSystem().then(() => {
    console.log('✅ Sistema de elevación inicializado correctamente');
}).catch(error => {
    console.warn('⚠️ Sistema iniciado en modo fallback:', error.message);
});

console.log('✅ Elevation handler para mini-tiles configurado');

// Configurar elevationHandler global para compatibilidad
window.elevationHandler = {
    obtenerElevacion: window.obtenerElevacion,
    isInitialized: () => isInitialized,
    diagnosticar: () => {
...
console.log('✅ Elevation handler v3.0 para mini-tiles configurado correctamente');
